...
RUN echo "deb http://deb.debian.org/debian trixie main contrib non-free non-free-firmware" > /etc/apt/sources.list \
    && apt-get update && apt-get install -y --no-install-recommends \
        ...
        # The actual GPU runtime is the Intel VPL GPU runtime (libmfx-gen),
        # packaged on trixie as libmfx-gen1.2. Covers Gen ≥9 incl. Arc.
        # intel-media-va-driver-non-free is the iHD driver — required for Arc
        # AV1 (encode + decode) and any Gen ≥9 QSV.
